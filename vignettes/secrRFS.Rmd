...
|------------------|---------------|------------|-------------|
Binary habitat mosaic | secr::randomHabitat | randomDensity | D, p, A
Log-gaussian random field | RandomFields::RFsimulate | randomGaussian | D, var, scale
...
library(secrRFS)
setNumThreads(4)   # adjust to number of available cores
```
...
This function generates an intensity surface as a log-gaussian random field using RFsimulate from the RandomFields package (Schlather et al. 2015).
...
# Scaling to constant $\E {N(A)}$

The parameter list for `randomDensity` and `randomGaussian` includes the component `maskscale`. When this is set to TRUE the intensity surface is scaled to provide a constant expected number within the area of `mask`. The default is `maskscale = FALSE`.

Scaling can have a large effect on $c$. For example, with `randomDensity`

```{r maskscale, cache = TRUE}
